...
const NavBar = () => {
  ...
  const currentLanguage = languages.find(lang => lang.code === i18n.language) || languages.find(lang => lang.code === 'cs');
    ...
    <div className="fixed top-0 bg-white z-50 w-full">
      <div className="nav shadow-md w-full max-w-8xl mx-auto flex items-center justify-between px-6 py-6">
        ...
        <nav className="hidden min-[799px]:flex gap-10">
          ...
          <NavLink to="/o-nas" className={linkClass}>{t('nav_about')}</NavLink>
          <NavLink to="/uhlie" className={linkClass}>{t('nav_products')}</NavLink>
        </nav>
              ...
              <div className="absolute right-0 mt-2 bg-white border border-gray-200 rounded shadow-md z-10">
                {languages.map(({ code, flag }) => (
                  <button
                    ...
                    className={`w-full px-2 py-2 hover:bg-gray-100 flex items-center ${i18n.language === code ? 'bg-gray-200' : ''}`}
                  >
                    <img
...
/>
          ...
          <NavLink to="/kontakt" className={contactClass}>{t('nav_contact')}</NavLink>
        </div>
      </div>

      <div className={`absolute z-50 top-full left-0 w-full bg-white shadow-md flex flex-col items-center py-6 transition-all duration-300 transform overflow-hidden min-[799px]:hidden ${isOpen ? 'max-h-96 opacity-100 translate-y-0' : 'max-h-0 opacity-0 -translate-y-2'}`}>
        <nav className="flex flex-col gap-6">
          <NavLink to="/" end onClick={() => setIsOpen(false)} className={linkClass}>{t('nav_home')}</NavLink>
          <NavLink to="/o-nas" onClick={() => setIsOpen(false)} className={linkClass}>{t('nav_about')}</NavLink>
          <NavLink to="/uhlie" onClick={() => setIsOpen(false)} className={linkClass}>{t('nav_products')}</NavLink>
        </nav>
      </div>
    </div>
  );
